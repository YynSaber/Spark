...
object CDNTest {
  ...
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setMaster("local[2]").setAppName("CDNTest")
    val sc = new SparkContext(conf)

    val input = sc.textFile("H://idea/cdn.txt").cache()
  ...
  def ipStatics(data: RDD[String]): Unit = {
    ...
    //1.统计独立IP数
    val ipNums = data.map(x => (IPPattern.findFirstIn(x).get, 1))
      .reduceByKey(_ + _)
      .sortBy(_._2, false)

    //输出IP访问数前量前10位
    ipNums.take(10).sortBy(_._1).foreach(println)
